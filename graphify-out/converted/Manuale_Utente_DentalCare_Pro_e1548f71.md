<!-- converted from Manuale_Utente_DentalCare_Pro.docx -->
...
DentalCare Pro
Manuale Utente
Percorsi guidati per Segreteria, Medico e Amministratore
...
Gestionale per studi odontoiatrici  ·  Edizione dimostrativa
...
Questo manuale è organizzato in tre percorsi guidati, uno per ciascun ruolo. Ogni persona che lavora in studio trova la propria sezione con le schermate reali dell'applicazione e i passi da seguire.
## 1.1  I tre ruoli


## 1.2  Accesso
Si accede da browser all'indirizzo dello studio. La schermata di login chiede email e password personali: ogni operatore ha le proprie credenziali.

Schermata di accesso. Ogni operatore usa le proprie credenziali personali.

- Aprire il browser all'indirizzo dello studio.
- Inserire la propria email e password.
- Premere «Accedi». Al primo accesso viene richiesto di scegliere una nuova password.
...
Dashboard della segreteria: indicatori della giornata, prossimi appuntamenti e scheda rapida del paziente.
...
Agenda in vista Giorno: le colonne sono le poltrone, i colori indicano lo stato dell'appuntamento.

## 2.3  Prenotare un appuntamento
- Dall'agenda premere «+ Appuntamento» in alto a destra.
- Cercare il paziente per nome o telefono; se è nuovo, crearlo al volo.
- Scegliere prestazione, medico, poltrona, data e ora.
- Salvare: l'appuntamento compare subito in agenda nella poltrona scelta.
...
Elenco pazienti: ogni card riassume contatti, visite, piani e richiami da fare.
...
Copilot AI: chat operativa a sinistra, contesto dello studio a destra (paziente, chiamate recenti, attività).
...
Dashboard del medico (Dr. Laura Ferretti): oltre agli indicatori, compare la voce «Prestazioni» nel menu.
...
Scheda paziente vista dal medico: tutti i tab clinici, allergie in evidenza, storico appuntamenti e dati anagrafici.
...
Odontogramma FDI: denti con carie e condizioni; il badge «A» segnala le condizioni suggerite dall'analisi AI, da verificare.
...
Cartella clinica: alert, riepiloghi clinico e anamnestico, stato dell'odontogramma e piani di cura in un colpo d'occhio.
...
Impostazioni · Studio: i dati fiscali dello studio, usati per la fatturazione. In alto i tab delle altre aree.
...
Prompt Manager: il comportamento dell'assistente AI è configurabile dall'amministratore, in chiaro e per lingua.
...
Prestazioni e Listino: il catalogo per categoria, con prezzo, durata e stato di ogni prestazione.
